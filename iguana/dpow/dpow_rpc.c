...
uint16_t Notaries_RPCport = IGUANA_NOTARYPORT;
...
cJSON *dpow_listunspent(struct supernet_info *myinfo,struct iguana_info *coin,char *coinaddr,int32_t utxosize)
{
    ...
    if ( coin->FULLNODE < 0 )
    {
        ...
        if ( utxosize != 0 )
        {
            ...
            if ( ++coin->utxocacheinit < 3 && coin->utxocacheactive == 0 && (retstr= bitcoind_passthru(coin->symbol,coin->chain->serverport,coin->chain->userpass,"dpowlistunspent", buf)) != 0 && (json= cJSON_Parse(retstr)) != 0 )
            {
            ...
            if ( retstr != 0 || (coin->utxocacheactive != 0 && (retstr= bitcoind_passthru(coin->symbol,coin->chain->serverport,coin->chain->userpass, "dpowlistunspent", buf)) != 0 && (json= cJSON_Parse(retstr)) != 0) )
            {
                ...
                {
                    coin->utxocacheinit--;
                    coin->utxocacheactive = 0;
                    free_json(json);
                    json = 0;
                }
                //printf("dpowlistspent %s buf.(%s) retstr.(%s)\n",coin->symbol,buf,retstr);
                free(retstr);
            } 
        }
...
char *dpow_sendrawtransaction(struct supernet_info *myinfo,struct iguana_info *coin,char *signedtx, int32_t mine, int32_t *errorcode)
{
    bits256 txid; cJSON *json,*array,*errorjson = 0,*retjson = 0; char *paramstr,*retstr;
    if ( coin->active == 0 ) return (0);
    if ( coin->FULLNODE < 0 )
    {
        array = cJSON_CreateArray();
        ...
        if ( errorcode != 0 && (retjson= cJSON_Parse(retstr)) != 0 && (errorjson= jobj(retjson,"error")) != 0 )
        {
            *errorcode = jint(errorjson,"code");
            return(0);
        }
        char colour[16];
